...
DROP POLICY IF EXISTS "Service role full access to campaigns" ON outreach_campaigns;
CREATE POLICY "Service role full access to campaigns" ON outreach_campaigns
  FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service role full access to contacts" ON outreach_contacts;
CREATE POLICY "Service role full access to contacts" ON outreach_contacts
  FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service role full access to email logs" ON outreach_email_logs;
CREATE POLICY "Service role full access to email logs" ON outreach_email_logs
  FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service role full access to templates" ON outreach_templates;
...
DROP POLICY IF EXISTS "Public can view published content" ON seo_content;
CREATE POLICY "Public can view published content" ON seo_content
  FOR SELECT USING (publish_status = 'published');

DROP POLICY IF EXISTS "Service role full access to seo content" ON seo_content;
...
DROP TRIGGER IF EXISTS update_seo_content_updated_at ON seo_content;
...
-- ============================================
INSERT INTO outreach_templates (name, type, subject_template, body_template) VALUES -- ON CONFLICT DO NOTHING equivalent below
